...
bool CheckersQueen::canCapture(GameState& state, int toX, int toY) const {
    ...
    if (state.getTileAt(toX, toY).getPiece() != nullptr) {
        ...
        //check if there is a piece adjacent to the destination square & if it is the opposite color
    int xDir = (toX - x) / abs(toX - x);
    int yDir = (toY - y) / abs(toY - y);
    if (state.getTileAt(toX - xDir, toY - yDir).getPiece() == nullptr && state.getTileAt(toX - xDir, toY - yDir).getPiece()->getColor() != color) {
        return false;
    }
    return true;
}
