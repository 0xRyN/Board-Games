...
IncognitoPawn::IncognitoPawn(int posX, int posY, Color pieceColor)
...
bool IncognitoPawn::canEnterToCastel(GameState& state, int toX, int toY) const {
    int castelCoordinates[2][2] = {{4, 0}, {0, 4}};

    // check if the move is diagonal
    if (abs(toX - x) != abs(toY - y)) {
        return false;
    }
    // check if the tile is the castel and the color of the piece
    for (int i = 0; i < 2; i++) {
        if (toX == castelCoordinates[i][0] && toY == castelCoordinates[i][1] &&
            color == (i == 0 ? Color::White : Color::Black)) {
            return true;
        }
    }
...
bool IncognitoPawn::Ask(GameState& state, int toX, int toY) const {
    ...
    if (abs(toX - x) > 1 || abs(toY - y) > 1) {
        std::cout << "You can only ask about adjacent orthogonally pieces"
                  << std::endl;
        return false;
    }

    const IncognitoPawn* pieceToAsk = const_cast<IncognitoPawn*>(
        dynamic_cast<const IncognitoPawn*>(state.getPiece(toX, toY)));
